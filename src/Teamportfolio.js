import React from "react";
import "./Teamportfolio.css";

function Teamportfolio() {
  return (
    <>
      <div className="top-bar"></div>
      <nav class="navbar navbar-expand-sm">
        <div class="container-fluid">
          <a class="navbar-brand" href="#">
            DevCom
          </a>
          <button
            class="navbar-toggler"
            type="button"
            data-bs-toggle="collapse"
            data-bs-target="#navbarNavAltMarkup"
            aria-controls="navbarNavAltMarkup"
            aria-expanded="false"
            aria-label="Toggle navigation"
          >
            <span class="navbar-toggler-icon"></span>
          </button>
          <div class="collapse navbar-collapse" id="navbarNavAltMarkup">
            <div class="navbar-nav">
              <a class="nav-link" aria-current="page" href="#">
                Home
              </a>
              <a class="nav-link" href="#">
                About
              </a>
              <a class="nav-link" href="#">
                Projects
              </a>
              <a class="nav-link" href="#">
                Team
              </a>
              <a class="nav-link" href="#">
                Connect
              </a>
            </div>
          </div>
        </div>
      </nav>
      <div className="outer-team-container">
        <div className="head-section">
          <div className="heading">
            <p className="headings">The Team</p>
          </div>
          <div className="content">
            <p className="contents">
              A group of highly enthusiastic and capable designers and
              developers, are are on the quest to change the student life at IIT
              Bombay
            </p>
          </div>
        </div>
        <div className="body-section">
          <div className="row OC-section">
            <div className="position">
              <p className="post">Overall Coordinators</p>
            </div>
            <div className="post-holders">
              <div className="member-box">
                <div className="member">
                  <p className="member-name">Adithya</p>
                  <div className="card">
                    <div className="front">
                      <p>hello</p>
                    </div>
                    <div className="back">
                      <p>mother-fucker!</p>
                    </div>
                  </div>
                </div>
              </div>
              <div className="member-box">
                <div className="member">
                  <p className="member-name">Palash</p>
                  <div className="card">
                    <div className="front">
                      <p>hello</p>
                    </div>
                    <div className="back">
                      <p>mother-fucker!</p>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div className="row project-lead-section">
            <div className="position">
              <p className="post">Project Leads</p>
            </div>
            <div className="post-holders">
              <div className="member-box">
                <div className="member">
                  <p className="member-name">Abhijat</p>
                  <div className="card">
                    <div className="front">
                      <p>hello</p>
                    </div>
                    <div className="back">
                      <p>mother-fucker!</p>
                    </div>
                  </div>
                </div>
              </div>
              <div className="member-box">
                <div className="member">
                  <p className="member-name">Dheer</p>
                  <div className="card">
                    <div className="front">
                      <p>hello</p>
                    </div>
                    <div className="back">
                      <p>mother-fucker!</p>
                    </div>
                  </div>
                </div>
              </div>
              <div className="member-box">
                <div className="member">
                  <p className="member-name">Krishna</p>
                  <div className="card">
                    <div className="front">
                      <p>hello</p>
                    </div>
                    <div className="back">
                      <p>mother-fucker!</p>
                    </div>
                  </div>
                </div>
              </div>
              <div className="member-box">
                <div className="member">
                  <p className="member-name">Rahul</p>
                  <div className="card">
                    <div className="front">
                      <p>hello</p>
                    </div>
                    <div className="back">
                      <p>mother-fucker!</p>
                    </div>
                  </div>
                </div>
              </div>
              <div className="member-box">
                <div className="member">
                  <p className="member-name">Sanskar</p>
                  <div className="card">
                    <div className="front">
                      <p>hello</p>
                    </div>
                    <div className="back">
                      <p>mother-fucker!</p>
                    </div>
                  </div>
                </div>
              </div>
              <div className="member-box">
                <div className="member">
                  <p className="member-name">Vedant</p>
                  <div className="card">
                    <div className="front">
                      <p>hello</p>
                    </div>
                    <div className="back">
                      <p>mother-fucker!</p>
                    </div>
                  </div>
                </div>
              </div>
              <div className="member-box">
                <div className="member">
                  <p className="member-name">Vidit</p>
                  <div className="card">
                    <div className="front">
                      <p>hello</p>
                    </div>
                    <div className="back">
                      <p>mother-fucker!</p>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div className="row design-lead-section">
            <div className="position">
              <p className="post">Design Leads</p>
            </div>
            <div className="post-holders">
              <div className="member-box">
                <div className="member">
                  <p className="member-name">Chilyang</p>
                  <div className="card">
                    <div className="front">
                      <p>hello</p>
                    </div>
                    <div className="back">
                      <p>mother-fucker!</p>
                    </div>
                  </div>
                </div>
              </div>
              <div className="member-box">
                <div className="member">
                  <p className="member-name">Tanmay</p>
                  <div className="card">
                    <div className="front">
                      <p>hello</p>
                    </div>
                    <div className="back">
                      <p>mother-fucker!</p>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div className="row Developers-section">
            <div className="position">
              <p className="post">Developers</p>
            </div>
            <div className="post-holders">
              <div className="member-box">
                <div className="member">
                  <p className="member-name">Akshat</p>
                  <div className="card">
                    <div className="front">
                      <p>hello</p>
                    </div>
                    <div className="back">
                      <p>mother-fucker!</p>
                    </div>
                  </div>
                </div>
              </div>
              <div className="member-box">
                <div className="member">
                  <p className="member-name">Aman</p>
                  <div className="card">
                    <div className="front">
                      <p>hello</p>
                    </div>
                    <div className="back">
                      <p>mother-fucker!</p>
                    </div>
                  </div>
                </div>
              </div>
              <div className="member-box">
                <div className="member">
                  <p className="member-name">Amit</p>
                  <div className="card">
                    <div className="front">
                      <p>hello</p>
                    </div>
                    <div className="back">
                      <p>mother-fucker!</p>
                    </div>
                  </div>
                </div>
              </div>
              <div className="member-box">
                <div className="member">
                  <p className="member-name">Ananya</p>
                  <div className="card">
                    <div className="front">
                      <p>hello</p>
                    </div>
                    <div className="back">
                      <p>mother-fucker!</p>
                    </div>
                  </div>
                </div>
              </div>
              <div className="member-box">
                <div className="member">
                  <p className="member-name">Asmi</p>
                  <div className="card">
                    <div className="front">
                      <p>hello</p>
                    </div>
                    <div className="back">
                      <p>mother-fucker!</p>
                    </div>
                  </div>
                </div>
              </div>
              <div className="member-box">
                <div className="member">
                  <p className="member-name">Chaitanya</p>
                  <div className="card">
                    <div className="front">
                      <p>hello</p>
                    </div>
                    <div className="back">
                      <p>mother-fucker!</p>
                    </div>
                  </div>
                </div>
              </div>
              <div className="member-box">
                <div className="member">
                  <p className="member-name">Chinmay</p>
                  <div className="card">
                    <div className="front">
                      <p>hello</p>
                    </div>
                    <div className="back">
                      <p>mother-fucker!</p>
                    </div>
                  </div>
                </div>
              </div>
              <div className="member-box">
                <div className="member">
                  <p className="member-name">Dipayan</p>
                  <div className="card">
                    <div className="front">
                      <p>hello</p>
                    </div>
                    <div className="back">
                      <p>mother-fucker!</p>
                    </div>
                  </div>
                </div>
              </div>
              <div className="member-box">
                <div className="member">
                  <p className="member-name">Harigovind</p>
                  <div className="card">
                    <div className="front">
                      <p>hello</p>
                    </div>
                    <div className="back">
                      <p>mother-fucker!</p>
                    </div>
                  </div>
                </div>
              </div>
              <div className="member-box">
                <div className="member">
                  <p className="member-name">Ishan</p>
                  <div className="card">
                    <div className="front">
                      <p>hello</p>
                    </div>
                    <div className="back">
                      <p>mother-fucker!</p>
                    </div>
                  </div>
                </div>
              </div>
              <div className="member-box">
                <div className="member">
                  <p className="member-name">Jatin</p>
                  <div className="card">
                    <div className="front">
                      <p>hello</p>
                    </div>
                    <div className="back">
                      <p>mother-fucker!</p>
                    </div>
                  </div>
                </div>
              </div>
              <div className="member-box">
                <div className="member">
                  <p className="member-name">Mahesh</p>
                  <div className="card">
                    <div className="front">
                      <p>hello</p>
                    </div>
                    <div className="back">
                      <p>mother-fucker!</p>
                    </div>
                  </div>
                </div>
              </div>
              <div className="member-box">
                <div className="member">
                  <p className="member-name">Nivedha</p>
                  <div className="card">
                    <div className="front">
                      <p>hello</p>
                    </div>
                    <div className="back">
                      <p>mother-fucker!</p>
                    </div>
                  </div>
                </div>
              </div>
              <div className="member-box">
                <div className="member">
                  <p className="member-name">Omkar</p>
                  <div className="card">
                    <div className="front">
                      <p>hello</p>
                    </div>
                    <div className="back">
                      <p>mother-fucker!</p>
                    </div>
                  </div>
                </div>
              </div>
              <div className="member-box">
                <div className="member">
                  <p className="member-name">Parth</p>
                  <div className="card">
                    <div className="front">
                      <p>hello</p>
                    </div>
                    <div className="back">
                      <p>mother-fucker!</p>
                    </div>
                  </div>
                </div>
              </div>
              <div className="member-box">
                <div className="member">
                  <p className="member-name">Saurabh</p>
                  <div className="card">
                    <div className="front">
                      <p>hello</p>
                    </div>
                    <div className="back">
                      <p>mother-fucker!</p>
                    </div>
                  </div>
                </div>
              </div>
              <div className="member-box">
                <div className="member">
                  <p className="member-name">Shivam</p>
                  <div className="card">
                    <div className="front">
                      <p>hello</p>
                    </div>
                    <div className="back">
                      <p>mother-fucker!</p>
                    </div>
                  </div>
                </div>
              </div>
              <div className="member-box">
                <div className="member">
                  <p className="member-name">Tanishka</p>
                  <div className="card">
                    <div className="front">
                      <p>hello</p>
                    </div>
                    <div className="back">
                      <p>mother-fucker!</p>
                    </div>
                  </div>
                </div>
              </div>
              <div className="member-box">
                <div className="member">
                  <p className="member-name">Tarus</p>
                  <div className="card">
                    <div className="front">
                      <p>hello</p>
                    </div>
                    <div className="back">
                      <p>mother-fucker!</p>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div className="row designers-section">
            <div className="position">
              <p className="post">Designers</p>
            </div>
            <div className="post-holders">
              <div className="member-box">
                <div className="member">
                  <p className="member-name">Sahil</p>
                  <div className="card">
                    <div className="front">
                      <p>hello</p>
                    </div>
                    <div className="back">
                      <p>mother-fucker!</p>
                    </div>
                  </div>
                </div>
              </div>
              <div className="member-box">
                <div className="member">
                  <p className="member-name">Shreekar</p>
                  <div className="card">
                    <div className="front">
                      <p>hello</p>
                    </div>
                    <div className="back">
                      <p>mother-fucker!</p>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </>
  );
}

export default Teamportfolio;
